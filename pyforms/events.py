...
from pyforms.commons import getWheelDelta, getKeyState, Area
from pyforms.enums import MouseButtonState, MouseButton, Keys, SizedPositions
...
class LVSelChangedEventArgs(EventArgs):
    __slots__ = ("item", "index", "isSelected" )
    def __init__(self, _item, _index, _isSel):
        self.item = _item
        self.index = _index
        self.isSelected = bool(_isSel)
        # print(f"EA created with num: {num}") 

class LVItemCheckEventArgs(EventArgs):
    __slots__ = ("item", "index", "checked" )
    def __init__(self, _item, _index, _isCheck):
        self.item = _item
        self.index = _index
        self.checked = bool(_isCheck)