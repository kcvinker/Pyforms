...
from pyforms.enums import ControlType, DateFormat
from pyforms.events import GEA, DateTimeEventArgs
from pyforms.apis import LPNMHDR, LPNMDATETIMECHANGE, SUBCLASSPROC
...
dtpDict = {}
# dtpStyle = con.WS_CHILD | con.WS_VISIBLE | con.WS_TABSTOP
OBJ_BRUSH = 0x00000002
common_styles = con.WS_TABSTOP|con.WS_CHILD|con.WS_VISIBLE
custom_date_style = common_styles|con.DTS_LONGDATEFORMAT|con.DTS_APPCANPARSE
    ...
    def createHandle(self):
        ...
        if self._hwnd:
            ...
            if self._format == DateFormat.CUSTOM_DATE:
                ...
                Control._smBuffer.fillBuffer(self._fmtString)
                api.SendMessage(self._hwnd, con.DTM_SETFORMATW, 0, Control._smBuffer.addr)
    ...
    def _setStyles(self):
        match self._format:
            case DateFormat.CUSTOM_DATE:
                self._style = custom_date_style
            case DateFormat.LONG_DATE:
                self._style = common_styles|con.DTS_LONGDATEFORMAT
            case DateFormat.SHORT_DATE:
                if self._4DYear:
                    self._style = common_styles|con.DTS_SHORTDATECENTURYFORMAT
                else:
                    self._style = common_styles|con.DTS_SHORTDATEFORMAT
            case DateFormat.TIME_ONLY:
                self._style = common_styles| con.DTS_TIMEFORMAT
        ...
        if self._showUpdown:
            # Set the DTS_UPDOWN style
            self._style |= con.DTS_UPDOWN
        else:
            # Clear the DTS_UPDOWN style
            self._style &= ~con.DTS_UPDOWN
