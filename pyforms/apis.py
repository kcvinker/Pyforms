...
from ctypes import (WINFUNCTYPE, 
                    Structure, windll, POINTER, 
                    c_void_p, c_char_p)
from ctypes.wintypes import (HICON, HWND, UINT, 
                             DWORD, LONG, HDC, LPCWSTR, 
                             LPWSTR, INT, HMENU, HINSTANCE, 
                             LPVOID, USHORT, HMODULE, ATOM, 
                             BOOL, HBRUSH, HGDIOBJ, HBITMAP, 
                             COLORREF, HPEN, HANDLE, BYTE, 
                             WCHAR, HFONT, WORD, HRGN, SHORT)
import ctypes as ct
import os
...
class NMITEMACTIVATE(Structure):
    ...
    def __str__(self) -> str:
        s1 = f"{self.iItem=}, {self.iSubItem=}, "
        s2 = f"{self.uOldState=}, {self.uNewState=}, {self.uChanged=}"
        return s1 + s2
...
class NMLISTVIEW(Structure):
    _fields_ = [
        ...
        ("uChanged", UINT),
        ("ptAction", POINT),
        ("lParam", LPARAM), # LPARAM is typically a LONG_PTR or a simple LONG depending on context (32/64-bit)
    ]
    def __str__(self) -> str:
        s1 = f"{self.iItem=}, {self.iSubItem=}, {self.uNewState=}, "
        s2 = f"{self.uOldState=}, {self.uChanged=}"
        return s1 + s2
...
class COPYDATASTRUCT(Structure):
    _fields_ = [
        ("dwData", ULONG_PTR), # app defined msg
        ("cbData", DWORD),  # size of lpData
        ("lpData", PVOID),  # data
    ]
LPCOPYDATA = POINTER(COPYDATASTRUCT)
