...
from ctypes import (WINFUNCTYPE, byref, sizeof, 
                    addressof, create_unicode_buffer)
from pyforms.control import Control
import pyforms.constants as con
from pyforms.commons import MyMessages, getMousePosOnMsg, pointInRect
from pyforms.enums import ControlType
from pyforms.events import GEA
from pyforms.apis import (LRESULT, UINT_PTR, DWORD_PTR, RECT, 
                            COMBOBOXINFO, WPARAM, LPARAM, SUBCLASSPROC)
...
cmbDict = {}
cmbTbDict = {}
# cmbStyle = con.WS_CHILD | con.WS_VISIBLE | con.WS_TABSTOP
...
class ComboBox(Control):
    ...
    def _insertItems(self):
        if self._items:
            for item in self._items:
                ...
                api.SendMessage(self._hwnd, con.CB_ADDSTRING, 0, Control._smBuffer.addr) #addressof(buff))
    ...
    def selectedItem(self):
        if self._isCreated:
            indx = self._sendMsg(con.CB_GETCURSEL, 0, 0)
            if indx >= 0:
                slen = self._sendMsg(con.CB_GETLBTEXTLEN, indx, 0)
                if slen > 0:
                    Control._smBuffer.ensureSize(slen)
                    self._sendMsg(con.CB_GETLBTEXT, indx, Control._smBuffer.addr)
                    return Control._smBuffer.toStr
        return None
    ...
    def enableInput(self, value: bool):
        ...
        if self._isCreated:
            if self._enableInput != value:
                if value:
                    self._style &= ~con.CBS_DROPDOWNLIST # remove it
                    self._style |= con.CBS_DROPDOWN # add it
                else:
                    self._style &= ~con.CBS_DROPDOWN 
                    self._style |= con.CBS_DROPDOWNLIST
    ...
    def addItem(self, item):
        ...
        if self._isCreated:
            sitem = item if isinstance(item, str) else str(item)
            Control._smBuffer.fillBuffer(sitem)
            api.SendMessage(self._hwnd, con.CB_ADDSTRING, 0, Control._smBuffer.addr)
    ...
    def removeItem(self, item):
        ...
        if self._items.__contains__(item):
            sitem = item if isinstance(item, str) else str(item)
            Control._smBuffer.fillBuffer(sitem)
            cIndex = api.SendMessage(self._hwnd, con.CB_FINDSTRING, -1, Control._smBuffer.addr)
            if cIndex > -1 :
                api.SendMessage(self._hwnd, con.CB_DELETESTRING, cIndex, 0)
    ...
    def addAndSelectItem(self, item):
        self.addItem(item)
        self.selectedIndex = len(self._items)
