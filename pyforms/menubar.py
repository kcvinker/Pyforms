...
from pyforms.commons import (
    MyMessages, getMousePosOnMsg, menuTxtFlag,
    StaticData, Font, getMousePos )

from pyforms.control import Control
from pyforms.colors import Color
from pyforms.events import GEA
from pyforms.enums import ControlType, MenuStyle, MenuItemEvents
...
class ContextMenu(MenuBase):
    ...
    def showMenu(self, lpm):
        try:
            ...
            getMousePos(pt, lpm)
            if pt.x == -1 or pt.y == -1:
                # ContextMenu message generated by a keybord shortcut.
                # So we need to find the mouse position in diff way.
                pt = getMousePosOnMsg()

            mid = TrackPopupMenu(self._handle, TPM_RETURNCMD, pt.x, pt.y, 
    ...
    def addHandler(self, menuName, func, event = MenuItemEvents.ON_CLICK):
        if len(self._menus):
            for menu in self._menus.values():
                if menu._text == menuName:
                    match event:
                        case MenuItemEvents.ON_CLICK:
                            menu.onClick = func
                        case MenuItemEvents.ON_POPUP:
                            menu.onPopup = func
                        case MenuItemEvents.ON_CLOSEUP:
                            menu.onCloseup = func
                        case MenuItemEvents.ON_FOCUS:
                            menu.onFocus = func
                    break
        
    def addMenuItem(self, menuText: str, callback):
        """Add a menu item to the context menu."""        
        mi = MenuItem(menuText, MenuType.MENU_ITEM, self._handle, self._menuCount)
        mi._isCmenu = True
        mi.onClick = callback
        self._menuCount += 1
        self._menus[mi._id] = mi
