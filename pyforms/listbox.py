...
from pyforms.apis import LRESULT, SUBCLASSPROC
import pyforms.apis as api
from pyforms.colors import Color

lbxDict = {}
lbxStyle = con.WS_BORDER|con.LBS_NOTIFY|con.LBS_HASSTRINGS
...
# -region Public functions
        ...
        if self._hwnd:
            ...
            if self._items:
                for item in self._items:
                    ...
                    api.SendMessage(self._hwnd, con.LB_ADDSTRING, 0, Control._smBuffer.addr)
    ...
    def insertItem(self, item, index):
        ...
        if self._isCreated:
            Control._smBuffer.fillBuffer(item)
            api.SendMessage(self._hwnd, con.LB_INSERTSTRING, index, Control._smBuffer.addr)
...
# -endregion Public functions
    ...
    def _getItem(self, index: int) -> str:
        ...
        if item_len != con.LB_ERR:
            Control._smBuffer.ensureSize(item_len)
            api.SendMessage(self._hwnd, con.LB_GETTEXT, index, Control._smBuffer.addr)
    ...
    def selectedItem(self, value):
        ...
        if self._isCreated and not self._multiSel:
            ...
            # Control._smBuffer.fillBuffer(value)
            api.SendMessage(self._hwnd, con.LB_SETCURSEL, index, 0)
    # #----------------------------------------------------------------------7
